function [normvfix,simvfix,stabvfix,stats] = gainstatplot(data,cellinds,titlestr,ploton,sepplots)
% Analyze gain changes between fixation and pursuit conditions in bighead
%
% Usage: [normvfix,simvfix,stabvfix,stats] = gainstatplot(data,cellinds,titlestr,ploton,sepplots)
%        [normvfix,simvfix,stabvfix,stats] = gainstatplot(data,cellinds,titlestr,ploton,sepplots)
...
for i = 1:3
%     numcellsl = sum(~isnan(gmat(:,l_inds(i))));
%     numcellsr = sum(~isnan(gmat(:,r_inds(i))));
%     cellCount = num2str(min([numcellsl numcellsr]));
    numcellsl = find(~isnan(gmat(:,l_inds(i))));
    numcellsr = find(~isnan(gmat(:,r_inds(i))));
    cellCount = num2str(numel(unique([numcellsl;numcellsr])));
    
    % Run sign-rank tests
    p_cond(i) = signrank([gmat(:,1);gmat(:,1)],[gmat(:,l_inds(i));gmat(:,r_inds(i))]);
    p_plot = round(p_cond(i),4,'significant');
    
    cond_med(i) = median([gmat(:,l_inds(i))-gmat(:,1);gmat(:,r_inds(i))-gmat(:,1)],'omitnan');
    cond_plot = round(cond_med(i),4,'significant');
    
    maxval = max(max(gmat));
    
    vals(:,i) = [gmat(:,l_inds(i));gmat(:,r_inds(i))] - [gmat(:,1);gmat(:,1)];
...
%         scatter(gmat(:,1),gmat(:,r_inds(i)),100,c,'filled');
        scatter(gmat(:,1),gmat(:,r_inds(i)),100,'filled',...'Marker','square',...
            'MarkerFaceColor',condColorsR{i},...[1 1 1],...
            'MarkerEdgeColor',condColorsR{i},...
            'MarkerFaceAlpha',0.7,...
            'MarkerEdgeAlpha',0.7);
        scatter(gmat(:,1),gmat(:,l_inds(i)),100,...'filled',...
        ...
        scax = gca;
        scax.XLabel.String = 'Amplitude (Fixation)';
        ...
        scax.YLabel.String = ['Amplitude (',names{i},' Pursuit)'];
...
%         cbar1.Label.String = 'Left-Right PTI';
        ...
        hold on;
        % Right
        h = histogram([gmat(:,l_inds(i))-gmat(:,1);gmat(:,r_inds(i))-gmat(:,1)],...
            'BinWidth',5,...
            'FaceColor',condColorsR{i});
        h.FaceAlpha = 0.7;
        % Left
        h2 = histogram(gmat(:,l_inds(i))-gmat(:,1),...
            'BinWidth',5,...
