function [normvfix,simvfix,stabvfix,stats] = bandstatplot(data,cellinds,titlestr,ploton,sepplots)
...
for i = 1:3
%     numcellsl = sum(~isnan(bandmat(:,l_inds(i))));
%     numcellsr = sum(~isnan(bandmat(:,r_inds(i))));
%     cellCount = num2str(min([numcellsl numcellsr]));
    numcellsl = find(~isnan(bandmat(:,l_inds(i))));
    numcellsr = find(~isnan(bandmat(:,r_inds(i))));
    cellCount = num2str(numel(unique([numcellsl;numcellsr])));
    
    % Run sign-rank tests
    p_cond(i) = signrank([bandmat(:,1);bandmat(:,1)],[bandmat(:,l_inds(i));bandmat(:,r_inds(i))]);
    p_plot = round(p_cond(i),4,'significant');
    
    cond_med(i) = median([bandmat(:,l_inds(i))-bandmat(:,1);bandmat(:,r_inds(i))-bandmat(:,1)],'omitnan');
    cond_plot = round(cond_med(i),4,'significant');
    
    maxval = max(max(bandmat));
    
    vals(:,i) = [bandmat(:,l_inds(i));bandmat(:,r_inds(i))] - [bandmat(:,1);bandmat(:,1)];
    ...
    if ploton
        ...
        subplot(1,2,1);
        hold on;
%         scatter(bandmat(:,1),bandmat(:,l_inds(i)),100,c,'filled','Marker','square');
%         scatter(bandmat(:,1),bandmat(:,r_inds(i)),100,c,'filled');
        scatter(bandmat(:,1),bandmat(:,l_inds(i)),100,'filled',...
            'MarkerFaceColor',condColorsR{i},...[1 1 1],...
            'MarkerEdgeColor',condColorsR{i},...
            'MarkerFaceAlpha',0.7,...
            'MarkerEdgeAlpha',0.7);
        scatter(bandmat(:,1),bandmat(:,r_inds(i)),100,'filled',...
        ...
        scax = gca;
        scax.XLabel.String = 'Bandwidth (Fixation)';
        ...
        scax.YLabel.String = ['Bandwidth (',names{i},' Pursuit)'];
...
%         cbar1.Label.String = 'Left-Right PTI';
        ...
        hold on;
        % Right
        h = histogram([bandmat(:,l_inds(i))-bandmat(:,1);bandmat(:,r_inds(i))-bandmat(:,1)],...
            'BinWidth',10,...
            'FaceColor',condColorsR{i});
        h.FaceAlpha = 0.7;
        % Left
        h2 = histogram(bandmat(:,l_inds(i))-bandmat(:,1),...
            'BinWidth',10,...
        ...
        xlabel('Bandwidth Difference (sp/s)','FontSize',20);
        ...
        hax.XAxis.MinorTickValues = -maxMinorTick:10:maxMinorTick;
